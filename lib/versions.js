...
Version.prototype.createNewVersion = function(key, signedVerhash) {
...
Version.prototype.createDeltaFrom = function(oldVersion) {
    console.log("IND", JSON.stringify(oldVersion._KEVs), JSON.stringify(this._KEVs));
    var deltas = [];
    for (var key in Object.keys(oldVersion._KEVs).sort()) {
        console.log("KEY IS", key);
        var mine = this._KEVs[key];
        if (mine)
            deltas.push(["set", key, mine]);
        else
            deltas.push(["del", key]);
    }
    return deltas;
...
function NewVersion(args) {
    ...
    this._KVs = (args.initialData && args.initialData.KVs) || {};
    this._KEVs = (args.initialData && args.initialData.KEVs) || {};
}
