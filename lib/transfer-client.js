...
Pusher.prototype._cycle = function() {
    ...
    var msg = { type: "push",
                from: this._oldVersion && this._oldVersion.getSignedVerhash(),
...
Pusher.prototype._response = function(resp) {
    ...
    if (resp.type === "missing-keys") {
        // server was interrupted, lost its mind. Start again from beginning.
        this.reset();
        return this._cycle();
    }
    if (resp.type === "bad-seqnum") {
        // bad seqnum (or out-of-date detected during applyDelta, but that
        // should be checked earlier by the server)
        ret = { type: "bad-seqnum" };
        return this._done.resolve(ret);
    }
...
function Puller(newVersionSignedVerhash, oldVersion, store, transport) {
    ...
    if (oldVersion)
        this._oldSignedVerhash = oldVersion.getSignedVerhash();
    else
        this._oldSignedVerhash = null;
...
Puller.prototype._response = function(resp) {
    ...
    if (resp.type === "unknown-delta") {
        if (this._oldSignedVerhash === null) {
